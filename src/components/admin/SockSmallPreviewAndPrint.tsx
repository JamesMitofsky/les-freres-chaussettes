...
export const SockSmallPreviewAndPrint: React.FC<{ product: CartProduct }> = ({ product }) => {
    ...
    const logo: string = product.customizationValues.find(element => element.field.id === fieldIds.image)?.value || "";
    ...
    const handleUpdateCustomizationLogo = async (url: string) => {
        const customizationValueId = product.customizationValues.find(element => element.field.id === fieldIds.image)?.id;
